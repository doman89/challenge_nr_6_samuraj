...
export class Character extends Common {
	...
	public constructor(name: string, store: RootState) {
		super();
		const characterInfo = store.characters.find(character => character.name === name) as Person;
		...
		this.species = this.parseSpeciesNames(characterInfo.species, store.species);
		this.starships = this.parseStarshipsNames(characterInfo.starships, store.starships);
		this.vehicles = this.parseVehiclesNames(characterInfo.vehicles, store.vehicles);

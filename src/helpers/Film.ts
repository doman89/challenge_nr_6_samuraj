...
export class Film extends Common {
	...
	public constructor(title: string, store: RootState) {
		...
		this.species = this.parseSpeciesNames(filmInfo.species, store.species);
		this.starships = this.parseStarshipsNames(filmInfo.starships, store.starships);
		this.vehicles = this.parseVehiclesNames(filmInfo.vehicles, store.vehicles);
		this.characters = this.parseCharactersNames(filmInfo.characters, store.characters);
